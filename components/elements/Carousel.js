...
const Carousel = ({cardList = [],carouselSize}) => {
  ...
  return (
    <div className="relative">
      <div style={style} className = "flex flex-row justify-start overflow-hidden" onTouchStart={(e)=> handleTouchStart(e)} onTouchMove={(e) => handleTouchMove(e)} >
      ...
      <div className="hidden sm:block absolute w-full top-1/2">
        <div onClick={() => handleRightMovement(cardList,carouselSize)} className="absolute -left-8 h-4 w-4 top-1/2 -translate-y-1/2 border-l-4 border-b-4 border-solid border-neon1 rotate-45 duration-200 hover:border-neon2"></div>
        <div onClick={() => handleLeftMovement(cardList,carouselSize)} className="absolute -right-8 h-4 w-4 top-1/2 -translate-y-1/2 border-r-4 border-t-4 border-solid border-neon1 rotate-45 duration-200 hover:border-neon2"></div>
      </div>
    </div>
  );
}
